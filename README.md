...
+ Mô tả các bảng:
	...
	-	``` sql	
		table Booking{ --// Đặt phòng
			...
			Status nvarchar(50) --// Trạng thái đặt phòng (Tentative, Confirmed, Cancelled, Checked-In, Checked-Out )
		}
		```
	-	``` sql	
		table BookingDetail{--//Chi tiết đặt phòng
			...
			Status nvarchar(50)--//Trạng thái (Tentative, Confirmed, Cancelled, Occupied, No-show, Checked-Out)
		}
		```
	-	``` sql	
		table Stay{--//Lưu trú
	...
	-	``` sql	
		table Guest{--//Khách hàng
			...
			GuarantorId int--//Mã người bảo lãnh
	...
	-	``` sql	
		table Invoice{--//hoa don khi checkout
			...
			AmountPaid int--//Tiền đã trả trươc (phí đặt phòng, tiền đặt cọc)
			Discount int--//Tiền khuyến mãi
			Total int --//Tổng tiền phải thanh toán 
